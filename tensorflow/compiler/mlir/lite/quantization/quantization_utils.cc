...
Type GetQuantizedType(Builder builder, Type input_type, ArrayRef<double> min,
  ...
  if (min.size() == 1 && max.size() == 1 && quant_dim == -1) {
    quantizedEleType = fakeQuantAttrsToTypeWith4BitSupport(
...
static bool getDefaultStorageParams(unsigned numBits, bool narrowRange,
  ...
  // FIXME Some tests failing with numBits <= 4
  if (numBits == 4) {
...
fakeQuantAttrsToTypeWith4BitSupport(Location loc, unsigned numBits, double rmin,
